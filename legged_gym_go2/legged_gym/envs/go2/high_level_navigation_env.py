...
class HighLevelNavigationEnv:
    ...
    def __init__(self, base_env: GO2Robot, cfg):
        ...
        self.lidar_max_range = getattr(self.cfg, "lidar_max_range", 10.0)
        self.target_lidar_max_range = getattr(self.cfg, "target_lidar_max_range", self.lidar_max_range)
        ...
        self.reach_metric_scale = float(getattr(self.cfg, "reach_metric_scale", 1.0))
    ...
    def _compute_high_level_observations(self):
        ...
        self.high_level_obs_buf[:, 2] = torch.clamp(base_lin_vel_body[:, 0] * self.lin_vel_scale, -1.0, 1.0)
        self.high_level_obs_buf[:, 3] = torch.clamp(base_lin_vel_body[:, 1] * self.lin_vel_scale, -1.0, 1.0)

        base_ang_vel_body = self.base_env.base_ang_vel  # body-frame angular velocity
        self.high_level_obs_buf[:, 4] = torch.clamp(base_ang_vel_body[:, 2] * self.ang_vel_scale, -1.0, 1.0)
        ...
        target_distance_norm = torch.clamp(target_distance / self.lidar_max_range, 0.0, 1.0)
        ...
        # Use true reach metric for obs[5] (optionally scaled).
        self.high_level_obs_buf[:, 5] = reach_metric * self.reach_metric_scale
        ...
        if self.use_manual_lidar and self.lidar_num_bins > 0:
            ...
            normalized_dist = torch.clamp(surface_dist / self.lidar_max_range, 0.0, 1.0)
    ...
    def extract_target_distance(self, obs: torch.Tensor) -> torch.Tensor:
        """Estimate target distance (meters) from the high-level observation."""
        if self.target_lidar_num_bins > 0:
            start = self._base_obs_dim
            end = start + self.target_lidar_num_bins
            target_bins = obs[:, start:end]
            max_intensity = torch.max(target_bins, dim=1).values
            surface_dist = (1.0 - max_intensity) * self.target_lidar_max_range
            return surface_dist + self.cfg.target_radius
        scale = self.reach_metric_scale if self.reach_metric_scale > 0 else 1.0
        return obs[:, 5] / scale

    def extract_hazard_distance(self, obs: torch.Tensor) -> torch.Tensor:
        """Estimate nearest hazard distance (meters) from obstacle/boundary lidar bins."""
        if not self.use_manual_lidar or self.lidar_num_bins <= 0:
            return torch.full((obs.shape[0],), self.lidar_max_range, device=obs.device)
        start = self._base_obs_dim + (self.target_lidar_num_bins if self.target_lidar_num_bins > 0 else 0)
        end = start + self.lidar_num_bins
        lidar_bins = obs[:, start:end]
        max_intensity = torch.max(lidar_bins, dim=1).values
        return (1.0 - max_intensity) * self.lidar_max_range

    def extract_body_vel_xy(self, obs: torch.Tensor) -> torch.Tensor:
        """Recover body-frame xy velocity from the normalized observation."""
        return obs[:, 2:4] / self.lin_vel_scale
    ...
    def _compute_boundary_lidar(self, rel_pos_xy, heading_cos, heading_sin):
        ...
        normalized = torch.clamp(surface_dist / self.lidar_max_range, 0.0, 1.0)
